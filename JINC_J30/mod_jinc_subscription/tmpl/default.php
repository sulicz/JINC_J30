...
foreach ($attrs_array as $attr_name => $attr_value) {
    if ($attr = $ninstance->loadAttribute($attr_name)) {
        if ($attr_value == ATTRIBUTE_MANDATORY) {
            echo 'var alert_msg = "' . JText::sprintf('COM_JINC_MANDATORY_ATTRIBUTE_JS', JText::_($attr->get('name_i18n'))) . '";';           
            echo 'if (!mod_validate_required(form.mod_' . $attr_name . ', alert_msg)) ';
            echo 'return false;';
        }
        if ($attr->get('type') == ATTRIBUTE_TYPE_INTEGER) {
            echo 'var alert_msg = "' . JText::sprintf('COM_JINC_INTEGER_ATTRIBUTE_JS', JText::_($attr->get('name_i18n'))) . '";';
            echo 'if (!mod_validate_integer(form.mod_' . $attr_name . ', alert_msg)) ';
            echo 'return false;';
        }
        if ($attr->get('type') == ATTRIBUTE_TYPE_DATE) {
            echo 'if ((!isEmpty(form.mod_' . $attr_name . '.value)) && (!isDate(form.mod_' . $attr_name . '.value))) {';
            echo 'alert("' . JText::sprintf('COM_JINC_DATE_ATTRIBUTE_JS', JText::_($attr->get('name_i18n'))) . '");';
            echo 'form.mod_' . $attr_name . '.focus();';
            echo 'return false;';
            echo '}';
        }
    } else {
...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" onsubmit="return mod_checkForm(this);" id="jinc_form_<?php echo $id; ?>" name="jinc_form_<?php echo $id; ?>">
    <div class="jinc_mod_frm_subscription">
        ...
        <?php if ($params->get('af_redir')): ?>
            <input type="hidden" name="af_redir" value="<?php echo $params->get('af_redir'); ?>">
        <?php endif; ?>        
