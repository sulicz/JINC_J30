...
class MessageFactory {
    ...
    function loadMessage($msg_id) {
        ...
        $query = 'SELECT m.id, m.body, m.subject, m.plaintext, ' .
            'm.attachment, m.bulkmail, m.news_id, m.tem_id ' .
    ...
    function loadTemplate($tem_id) {
        ...
        $query = 'SELECT name, subject, body, cssfile ' .
        ...
        $template->set('cssfile', $template_info['cssfile']);
        return $template;
    }
    ...
    function loadHistory($msg_id) {
        ...
        $query = 'SELECT id, status, UNIX_TIMESTAMP(start_time) AS start_time, ' .
            'UNIX_TIMESTAMP(last_update_time) AS last_update_time, client_id, sent_messages, sent_success, ' .
            'UNIX_TIMESTAMP(last_subscriber_time) AS last_subscriber_time, last_subscriber_id, 1-ISNULL(r.proc_id) as has_report ' .
            'FROM #__jinc_process p ' .
            'LEFT JOIN #__jinc_report r ON p.id = r.proc_id ' .
            'WHERE msg_id = ' . (int) $msg_id . ' ' .
            'AND status = ' . PROCESS_STATUS_FINISHED . ' ' .
            'GROUP BY id '.
        ...
        if ($result = $dbo->loadAssocList()) {
            foreach ($result as $info) {
                ...
                $process->set('sent_messages', $info['sent_messages']);
                $process->set('sent_success', $info['sent_success']);
                $process->set('has_report', intval($info['has_report']) == 1);
    ...
    /**
     * The message report data. It returns an array representig the sending
     * report related to report identifier passed as parameter.
     *
     * @access	public
     * @param	integer $proc_id the process identifier.
     * @return  array   Array of report results or false if something wrong
     * @since	1.0
     * @see     Process
     */

    function loadReport($proc_id) {
        jincimport('utility.servicelocator');
        $servicelocator = ServiceLocator::getInstance();
        $logger = $servicelocator->getLogger();

        $query = 'SELECT COALESCE(u.email,s.email) as email, error_code ' . 
                'FROM `#__jinc_report` r ' . 
                'LEFT JOIN #__jinc_subscriber s on r.subs_id = s.id ' . 
        ...
        if ($result = $dbo->loadAssocList()) {
            foreach ($result as $info) {
                array_push($report, array($info['email'] => $info['error_code']));
            }
        } else {
            return false;
        }
