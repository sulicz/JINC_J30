        ...
        if ($newsletter = $this->loadNewsletter()) {
            ...
            foreach ($arr_attachment as $key => $value) {
                ...
                if (strlen($attachment)) {
                    $logger->finer('PersonalMessage: adding attachment ' . $path_abs_root . DIRECTORY_SEPARATOR . $attachment);
            ...
            for ($i = 0; $i < $nrecips && $success; $i++) {
                ...
                if (!$mailmsg->addRecipient($current['email'])) {
                    $this->reported_recipients[$current['id']] = 1;
                }

                $mailmsg->setBody($this->emogrify($body));
                ...
                if ($send_mail) {
                    ...
                    if ($success === true) {
                        ...
                        $logger->warning('PersonalMessage: error sending mail. MSG = ' . $this->mail_system_error);
                        $this->setError('COM_JINC_ERR049');
