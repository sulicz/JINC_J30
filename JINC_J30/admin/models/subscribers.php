...
class JINCModelSubscribers extends JModelList {
    ...
    protected function populateState() {
        ...
        $filterSubState = $app->getUserStateFromRequest($this->context . '.filter.sub_state', 'filter_sub_state', '');
        $this->setState('filter.sub_state', $filterSubState);
    ...
    protected function getListQuery() {
        ...
        $search = $this->getState('filter.sub_state');
        if ($search == '0')
            $query->where('LENGTH(random) = 0');
        if ($search == '1')
            $query->where('LENGTH(random) > 0');
