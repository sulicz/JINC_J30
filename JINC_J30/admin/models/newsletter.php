...
class JINCModelNewsletter extends JModelAdmin {
    ...
    /**
     * Method to get the addsubscriber record form.
     *
     * @param	array	$data		Data for the form.
     * @param	boolean	$loadData	True if the form is to load its own data (default case), false if not.
     *
     ...
     * @since	1.6
     */
    public function getAddsubscriberForm($data = array(), $loadData = true) {
        $form = $this->loadForm('com_jinc.subscriber', 'addsubscriber', array('load_data' => false));
        $data = array();
        if ($loadData) {
            $data = $this->loadFormData();
        }
