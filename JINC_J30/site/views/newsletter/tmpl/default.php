...
foreach ($attrs_array as $attr_name => $attr_value) {
    if ($attr = $ninstance->loadAttribute($attr_name)) {
        ...
        if ($attr_value == ATTRIBUTE_MANDATORY) {
            ...
            echo 'alert("' . JText::sprintf('COM_JINC_MANDATORY_ATTRIBUTE_JS', JText::_($attr->get('name_i18n'))) . '");';
        ...
        if ($attr->get('type') == ATTRIBUTE_TYPE_INTEGER) {
            ...
            echo 'alert("' . JText::sprintf('COM_JINC_INTEGER_ATTRIBUTE_JS', JText::_($attr->get('name_i18n'))) . '");';
            echo '' . $attr_name . '_field.focus();';
            echo 'return false;';
            echo '}';
        }
...
<?php
if ($newsletter->get('suspended') != SUSPEND_BOTH && $newsletter->get('suspended') != SUSPEND_SUBSCRIPTION) {
    ?>
